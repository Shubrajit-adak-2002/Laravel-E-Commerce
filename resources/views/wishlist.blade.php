                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="alert alert-danger">
                                <strong>Item not found in wishlist!</strong>
                                <a href="{{route('shop.index')}}" class="btn btn-info">Add items to wishlist</a>
                            </div>
                        </div>
                    </div>
                @else
                            ...
                            <tbody>
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{route('wishlist.move',['id'=>$item->rowId])}}" method="post">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-warning">Move to Cart</button>
                                            </form>

                                            <form action="{{ route('wishlist.item.remove', ['id' => $item->rowId]) }}"
